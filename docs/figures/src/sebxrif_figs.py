...
DSRM_PHASES = [
    ("1. Problem identification", "SLR gaps, passive 2D, N=30"),
    ("2. Data acquisition", "xAPI 480 records, public, complete"),
    ("3. Proposed solution", "SEB-XRIF, SWOT, four layers"),
    ("4. Design and development", "Random Forest, dashboard"),
    ("5. Evaluation and results communication", "Accuracy, SUS, d, report"),
]

PIPELINE_STAGES = [
    ("Raw xAPI data", "480 records, 16 features"),
    ("Preprocessing", "Complete, one-hot, numeric counts"),
    ("Stratified split", "80/20, fixed seed"),
    ("Random Forest", "100 trees, balanced weights"),
    ("Model outputs", "Tiers, metrics, importance"),
    ("Dashboard", "Flask, React, SUS, Cohen d"),
]
...
def flow_figure(boxes, base_name, iteration_arrow=False, ncols=3):
    ...
    for i, (t_lines, d_lines) in enumerate(wrapped):
        ...
        if c > 0:
            ...
            ax.annotate(
                ...
                arrowprops=dict(arrowstyle="-|>", color=ARROW_C, linewidth=1.5),
            )
        elif r > 0:
    ...
    plt.close(fig)
    print(
        f"Saved {base_name}.png and {base_name}.pdf "
