...
COLORS = {"VR": "#4472C4", "AR": "#ED7D31", "MR": "#70AD47", "MR/AR": "#9E6BB5",
          "XR": "#17A2B8", "Immersive sim.": "#7F7F7F"}
...
bars = ax.bar(ORDER, [counts.get(m, 0) for m in ORDER],
...
ax.set_title("Immersive methods used across the 28 reviewed studies", fontsize=12, pad=12)
...
for m in ORDER:
    if not groups.get(m):
        continue
    axl.text(0.02, y, f"{m} ({counts[m]})", transform=axl.transAxes,
             fontsize=10, fontweight="bold", color=COLORS[m], va="top")
    y -= step * 1.15
    for nm in sorted(groups[m], key=lambda s: int(s.split("]")[0][1:])):
        axl.text(0.06, y, "\u2022 " + nm, transform=axl.transAxes,
